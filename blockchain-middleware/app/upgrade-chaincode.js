...
var upgradeChaincode = async function(peers, channelName, chaincodeName, chaincodeVersion, functionName, chaincodeType, args, username, org_name) {
	...
	if (channelName ==="channelyblanchorvendor"){
		endorsement_policy = {
			identities: [
			{ role: { name: 'member', mspId: 'YblMSP' }},
			{ role: { name: 'member', mspId: 'AnchorMSP' }},
                        { role: { name: 'member', mspId: 'VendorMSP' }}
			],
			policy: {
			'1-of':[{ 'signed-by': 0 }, { 'signed-by': 1 }, { 'signed-by': 2}]
			}
		}
	} else if (channelName ==="channelsupman"){
		endorsement_policy = {
			identities: [
			{ role: { name: 'member', mspId: 'SupplierMSP' }},
	...
	} else if (channelName ==="channelsupipdc"){
		endorsement_policy = {
			identities: [
			{ role: { name: 'member', mspId: 'SupplierMSP' }},
			{ role: { name: 'member', mspId: 'IpdcMSP' }}
			],
			policy: {
			'1-of':[{ 'signed-by': 0 }, { 'signed-by': 1 }]
			}
		}
	} else if (channelName ==="channelmanipdc"){
                endorsement_policy = {
                        identities: [
                        { role: { name: 'member', mspId: 'ManufacturerMSP' }},
                        { role: { name: 'member', mspId: 'IpdcMSP' }}
        ...
        } else if (channelName ==="channelipdcmandis"){
                endorsement_policy = {
                        identities: [
                        { role: { name: 'member', mspId: 'ManufacturerMSP' }},
                        { role: { name: 'member', mspId: 'IpdcMSP' }},
        ...
        } else if (channelName ==="channeldisipdc"){
                endorsement_policy = {
                        identities: [
                        { role: { name: 'member', mspId: 'DistributorMSP' }},
                        { role: { name: 'member', mspId: 'IpdcMSP' }}
	...
	if (!error_message) {
		let message = util.format(
			'Successfully upgraded chaincode in organization %s to the channel \'%s\'',
